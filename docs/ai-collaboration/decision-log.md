...
## ADR-016. M4-a 배송완료 → 정산-가능 이벤트 발행 (트랜잭셔널 아웃박스) ★

- **맥락**: ADR-015가 scope-out한 "DELIVERED 시 정산-가능 신호(outbox 이벤트) 발행"을 채운다. settlement가 소비하는 M4의 진입점이며, 이 슬라이스는 **shipping BC 1개 단위**(발행까지). 소비(settlement_line 생성)는 M4-b.
- **결정 1 — DELIVERED 전이와 이벤트 적재를 한 트랜잭션으로**: `recordTransition`이 `target == DELIVERED`일 때 잠금을 쥔 같은 트랜잭션에서 `outbox.append("shipment", shipmentId, "ShipmentDelivered", payload)`를 호출한다(dual-write 원자화, ADR-002/013 선례). 상태 전이가 롤백되면 이벤트도 함께 롤백된다.
- **결정 2 — 정확히 1회 발행은 구조가 보장(별도 멱등키 없음)**: DELIVERED는 종료 상태라 상태머신이 재진입을 막고(재-POST는 append 전에 409), 핫로우 `FOR UPDATE`가 동시 전이를 직렬화하므로 이벤트는 배송당 정확히 1회다 — payment의 `isPaid` no-op과 같은 원리. `uq_outbox_event(aggregate_type, aggregate_id, event_type)`가 최후 보루. `OutboxAppender`는 plain INSERT(ON CONFLICT 아님)라 이 구조가 깨지면 23505로 fail-loud.
- **결정 3 — payload는 primitive + ISO-8601 문자열 timestamp**: `{shipmentId, orderId, sellerId, deliveredAt}`. published 경계는 primitive를 쓴다(ADR-012 결)와 정합 — `deliveredAt`을 `OffsetDateTime`이 아닌 ISO 문자열로 실어 `jackson-datatype-jsr310` 모듈 의존을 피하고 소비자 파싱을 단순화. `deliveredAt`은 이벤트 INSERT의 `RETURNING occurred_at`으로 **DB가 확정한 값**(택배사 타임스탬프 또는 now())을 그대로 실어 저장 행과 이벤트가 어긋나지 않게 한다. `order_id`·`seller_id`는 상태 잠금(`FOR UPDATE`) 쿼리에서 함께 읽는다(셀러 귀속 = settlement_line.seller_id 근거).
- **의도적 Scope Out**: settlement 소비(`ShipmentDeliveredHandler` → 셀러별 `settlement_line` SALE(+)/COMMISSION(−) 적재)는 **M4-b**. 수수료 정책·정산 주기 마감·순지급 집계도 후속.
- **검증**: 인수 IT(`ShipmentTransitionIT.DeliveredEvent` — DELIVERED 1건 발행+payload 셀러 귀속 확인 / 중간 전이·FAILED 미발행 / 재-POST 409에도 여전히 1건) + 동시성 IT(`ShipmentTransitionConcurrencyIT` — 16스레드 동시 DELIVERED → success 1·이벤트 정확히 1·uq 최후보루 미의존) 전부 실 Postgres 그린.

## ADR-017. M4-b 배송완료 소비 → 셀러 정산 라인(SALE/COMMISSION) 적재 ★

- **맥락**: M4-a가 발행한 `ShipmentDelivered`를 settlement가 소비해 셀러별 정산 원장을 쌓는다(S5). 한 배송건 = 한 셀러의 한 주문 몫이므로 매출 SALE(+)과 수수료 COMMISSION(−) 두 줄을 append한다. settlement BC 1개 단위.
- **결정 1 — 매출 금액은 order.published 포트로 조회**: `ShipmentDelivered` payload엔 금액이 없다(shipmentId·orderId·sellerId·deliveredAt만). settlement가 `order.published.OrderForSettlement.sellerAmountForOrder(orderId, sellerId)` = Σ(해당 셀러 line_amount)로 SALE 금액을 얻는다(shipping의 `OrderForShipping` 선례와 동형, ArchUnit이 settlement→order.published 단방향 강제). 대안(이벤트에 금액 실기)은 payload를 무겁게 하고 발행측이 정산 관심사를 알게 돼 기각.
- **결정 2 — 수수료율은 commission_policy 테이블 + 코드 기본율**: `commission_policy(seller_id, rate_bps)`(V10)에서 셀러율을 읽고, 없으면 `DEFAULT_RATE_BPS`(10%). `rate_bps` CHECK 0..10000(0%..100%)가 |COMMISSION| ≤ SALE 을 DB로 보장 → 셀러 순지급(SUM)이 음수로 안 떨어진다. `commission = floor(amount × rate_bps / 10000)`, 오버플로는 `multiplyExact`로 fail-loud. rate 0%나 반올림으로 commission=0이면 COMMISSION 라인을 만들지 않는다(`ck_settline_sign`은 COMMISSION<0 요구).
- **결정 3 — 멱등은 (source_event_id, entry_type, seller_id) UNIQUE + ON CONFLICT DO NOTHING**: `source_event_id = "shipment:{shipmentId}"`, `source_type='ORDER_ITEM'`, `source_id=shipmentId`, `eligible_at=deliveredAt`, `cycle_id=NULL`(마감 시 귀속, 후속). at-least-once 재전달을 no-op으로 흡수(payment/shipping 선례). SALE·COMMISSION은 entry_type이 달라 같은 event_id로 fan-out 허용(ADR-010 복합키).
- **의도적 Scope Out**: 정산 주기(settlement_cycle) 마감·cycle_id 귀속·순지급 집계, 환불(REFUND 음수 라인, M5), 4-way 대사(M6). 릴레이는 `outbox.relay.enabled` 기본 off라 M4-b 배포 시 핸들러와 함께 켜야 선소모가 없다(테스트는 디스패처 직접 호출).
- **검증**: 통합 IT(`SettlementFromDeliveryIT` — 정책률 SALE+/COMMISSION− 생성 / 기본율 fallback / 재소비 멱등 / 주문 미가시 시 0줄) + property(`CommissionPropertyTest` — 0≤commission≤amount·순지급≥0·수수료율 단조성) 전부 실 Postgres 그린. ArchUnit(경계·순환) 통과.

## ADR-018. M4-c 정산 사이클 마감 배치 — EXCLUDE + 조건부 UPDATE 직렬화, ADR-005 락 비대칭 재검토 ★

- **맥락**: ADR-017이 후속으로 미뤄둔 "정산 주기 마감·`cycle_id` 귀속·순지급 조회"를 채운다(S5 후반). 배송완료 소비가 `cycle_id=NULL`로 쌓아둔 미배정 라인을 셀러별 사이클에 귀속시키고(`POST /admin/settlements/run`), 셀러 정산을 읽는다(`GET /sellers/{id}/settlements`). settlement BC 1개 단위.
- **결정 1 — cutoff 스윕(하한 없는 창)**: 마감은 `eligible_at < periodEnd` 인 미배정 라인을 전부 잡는다(하한 `>= periodStart` 없음). *처음엔 `[periodStart, periodEnd)` 양변 필터로 짰다가 적대적 검증에서 결함 발견* — 하한을 두면 이미 닫힌 과거 창에 늦게 도착한 라인(택배사 콜백 지연으로 `eligible_at`이 과거)이 어느 마감에도 안 걸려 **영구 고립**된다(닫힌 창은 EXCLUDE로 재마감 불가). cutoff 스윕은 늦은 라인을 다음 마감이 자연히 흡수한다(손실 0). `periodStart`는 EXCLUDE가 강제하는 보고 주기 라벨일 뿐 라인 선별 기준이 아니다. 반례를 `lateArrivalPickedUpByNextClose`로 회귀 고정.
- **결정 2 — 이중 마감 방어는 코드 락이 아니라 DB 구조 두 겹**: ① **이중 집계 금지** = `ex_cycle_no_overlap` EXCLUDE(GiST, ADR-010). 같은 셀러·유형·겹치는 창을 두 번째로 마감하려는 `settlement_cycle` INSERT가 거부돼 최대 1건만 커밋(`@Transactional`이라 패자 배치는 전체 롤백 — 부분 마감 없음). ② **라인 귀속 배타성** = `WHERE cycle_id IS NULL` 조건부 UPDATE. 조건부 UPDATE가 대상 행에 배타락을 잡고, 이미 귀속된 라인은 조건 불충족으로 재귀속되지 않는다. 셀러 사이클 INSERT는 `ORDER BY seller_id`로 EXCLUDE 인덱스 락 획득 순서를 고정(데드락 예방).
- **결정 3 — ADR-005(유입=FOR SHARE / 마감=FOR UPDATE)를 이 경로엔 적용하지 않는다(정직한 재검토) ★**: ADR-005의 락 비대칭은 *eager-assignment*(유입 시 열린 사이클 행에 즉시 귀속) 모델을 전제했다. 그러나 ADR-017이 *deferred-assignment*(라인은 `cycle_id=NULL`로 유입, 마감이 스윕 귀속)를 채택하면서 **유입과 마감이 같은 사이클 행을 공유하지 않게 됐다** — 유입은 라인 INSERT만 하고 사이클 행을 건드리지 않는다. 공유 자원이 없으므로 상호배제할 락 자체가 필요 없고, 대신 EXCLUDE(사이클)와 조건부 UPDATE(라인)가 직렬화를 맡는다. ADR-005의 통찰("shared 락끼리는 상호배제가 안 된다")은 여전히 옳지만 *적용 지점이 사라졌다*. deferred 모델의 대가는 마감 직전까지 라인이 미배정으로 떠 있다는 것(조회에서 pending으로 노출)인데, 정산은 배치성이라 수용 가능하다.
- **리뷰 반영(PR #24) — TOCTOU 고스트 사이클 ★**: CodeRabbit이 Major로 적중. 후보 셀러 조회와 셀러별 UPDATE 사이에 *다른 cycle_type*(EXCLUDE는 seller+type+기간 키라 유형이 다르면 안 막음) 마감이 같은 미배정 라인을 선점하면, UPDATE가 0건인데 사이클 INSERT는 이미 성공해 **라인 0개짜리 CLOSED 고스트**가 (seller,type,기간)을 영구 봉인한다. 8스레드 크로스-타입 동시 마감으로 **8/8 재현**(반례를 `concurrentCrossTypeCloseLeavesNoGhostCycle`로 박제) → `assigned==0`이면 방금 만든 사이클을 같은 TX에서 DELETE하는 가드로 봉쇄. settlement_cycle은 원장이 아니라 기간 메타라 DELETE가 잔액-UPDATE-금지 규율과 무관하고, 이 경로가 유일한 writer(ADR-015 "write 경로 하나면 앱 강제"와 정합). DB 트리거(빈 CLOSED 금지)는 지연제약 문제라 과설계로 보류. "AI 산출물을 적대적으로 검증"이 봇 리뷰에서 또 작동한 사례(ADR-005/008/013 계열).
- **리뷰 반영(PR #24) — 기타 판정**: ① 읽기 모델 방어복사(CodeRabbit, ADR-012 CartSnapshot 선례) **수용** — `SellerSettlementView`에 `List.copyOf`. ② IDOR 인가 가드(CodeRabbit Major) **defer** — 전역 인증/인가가 아직 stub(로드맵 "유저·인증·인가")이라 order/payment/shipping 전 엔드포인트가 열려 있다. 이 경로만 소유권 체크는 부정합이며 검증할 principal도 없다. ③ `CycleSummary` API 노출(CodeRabbit) **거절** — `OrderView`·`ShipmentView`·`SellerSettlementView`를 컨트롤러가 그대로 반환하는 확립된 관례와 동급(ADR-011 리치 뷰). ④ writable CTE 단일문·서브쿼리 재작성(Gemini high/medium) **defer** — 배치 경로의 3N+1은 OLTP가 아니라 ADR-004 "측정이 정당화할 때만". 단일 CTE는 크로스-타입 고스트를 못 막고(같은 스냅샷에서도 다른 유형 UPDATE는 0건) 셀러별 루프의 락 순서 서사를 흐린다.
- **의도적 Scope Out**: 2단계 마감(OPEN→CLOSING→CLOSED)·`CARRIED_OVER` 이월 상태(현재는 CLOSED 단일 + cutoff 스윕으로 late-arrival 흡수라 이월 상태머신 불필요), 지급대행(payout 게이트), 스케줄 자동 마감(현재 운영자 트리거), 인증/인가. 환불 음수 라인(M5)·4-way 대사(M6)는 다음 마일스톤.
- **검증**: 통합 IT(`SettlementCloseIT` — 귀속/창밖 제외/겹침 재마감 EXCLUDE 롤백/late-arrival 흡수), 동시성 IT(`SettlementCloseConcurrencyIT` — 16스레드 동시 마감 → 사이클 정확히 1개·이중집계 0, 5회 반복 무결함), property(`SettlementConservationPropertyTest`, 200 tries — 마감은 원장 SUM 불변 + **Σ셀러정산 + Σ수수료 == 총매출** + 순지급≥0), 인수 IT(`SettlementAcceptanceIT` — API 200/400/409 매핑 + 조회 읽기 모델) 전부 실 Postgres 그린. ArchUnit(settlement→order.published 단방향·순환 없음) 통과.

## ADR-019. M5-a 부분환불 — append-only refund 원장 + `0≤누적환불≤결제` 트리거 + 핫로우 직렬화 ★

- **맥락**: 골든 시나리오 S6. 이미 잡힌 매출을 되돌리는 환불의 결제측 절반(payment BC 1개 단위). 정산 REFUND 음수 라인 적재는 M5-b(settlement)로 분리(M4의 a/b/c 분할과 동형).
- **결정 1 — 잠정 컬럼을 걷어내고 append-only 원장으로(옵션 B 채택)**: V2의 `payment.refunded_amount`(+`ck_payment_refund_range`·`ck_payment_refund_status`)를 드롭하고, 환불을 `refund` 테이블에 한 행씩 쌓는다(V11). 누적환불 = `SUM(refund.amount_minor)` 도출 — AGENTS.md "잔액 컬럼 UPDATE 금지"·ADR-010 원장 철학과 정합. V2 주석이 예고한 전환을 실현. 대안 A(컬럼 UPDATE 유지)는 잔액-UPDATE 황금률과 충돌하고 동시 환불 lost-update 위험이 있어 기각.
- **결정 2 — 불변식 `0 ≤ 누적환불 ≤ 결제금액`은 코드가 아니라 두 DB 구조로 강제**: 하한(>0)은 `ck_refund_amount_positive` CHECK. 상한(≤ paid)은 여러 환불에 걸친 합이라 단일 CHECK로 못 박으므로 `trg_refund_within_paid` BEFORE INSERT 트리거가 `SUM(환불)+NEW ≤ paid_amount`를 강제(주문 총액=Σ라인을 트리거로 박은 ADR-008과 같은 계열, `check_violation`→409). 멱등은 `uq_refund_event(source_event_id)` UNIQUE.
- **결정 3 — 따닥 환불은 payment 행 `FOR UPDATE`로 직렬화(ADR-005 핫로우)**: 트리거만으로는 동시 환불의 phantom(서로의 미커밋 행을 못 봄)을 못 막는다. `refund` 서비스가 payment 행을 `FOR UPDATE`로 잠근 뒤 상태·금액을 읽어, 앞 환불의 커밋분을 SUM에 반영한 트리거가 초과를 거부하게 한다. 16스레드 동시 300원 환불(paid 1000)이 정확히 3건만 통과함을 실증(5회 반복 무결함).
- **결정 4 — 멱등은 payment_id로 스코프한 no-op + 전역 UNIQUE 최후보루**: 잠금 안에서 `(source_event_id, payment_id)` 존재를 확인해 정당한 재시도는 PG 재호출 없이 no-op(confirm의 isPaid 선례). *적대적 검증에서 정정*: 전역으로만 걸면 다른 주문에 같은 토큰을 재사용했을 때 엉뚱한 결제 상태를 no-op 반환한다 → payment_id로 좁혀 교차 오용은 `uq_refund_event`(전역)가 INSERT에서 409로 드러내게 했다.
- **결정 5 — PaymentRefunded 이벤트의 aggregate는 결제가 아니라 refund**: 한 결제에 환불이 여러 번이라 `aggregate_id=paymentId`면 `uq_outbox_event(aggregate_type,aggregate_id,event_type)`가 둘째 환불의 발행을 막는다(테스트로 재현). `aggregate_type='refund', aggregate_id=refundId`로 분리해 환불마다 정확히 1건 발행. 소비 라우팅은 event_type 기준이라 영향 없다.
- **경계·엔드포인트**: `refund.payment_id → payment(id)`는 같은 BC라 FK. `order_line_id`는 하류 정산 귀속(셀러)용 타 BC 논리 참조(FK 아님). 엔드포인트 `POST /orders/{orderId}/refunds`는 고객 모델(주문)을 따르나 로직은 payment BC — 결제를 `order_id`로 찾아 자기 테이블만 다룬다(order BC 미참조, ArchUnit 통과). **한 주문 = 한 결제** 전제(PlaceOrder→confirm 흐름). 이벤트 payload는 `{paymentId, orderId, orderLineId, amountMinor, refundId}`(primitive, ADR-016 계열).
- **의도적 Scope Out**: 정산 REFUND(−)·REFUND_COMMISSION(+) 라인 적재(M5-b), 라인별 상한(`누적환불(line) ≤ line_amount` — order 데이터 필요), 환불 사유·부분취소 상태머신, 교환/반품 회수 물류.
- **검증**: 예제 IT(`RefundIT` — 부분환불 누적·초과 거부·미결제 거부·미존재 404·멱등 no-op), DDL 제약 IT(`RefundConstraintsIT` — CHECK·트리거·UNIQUE·FK), property(`RefundLedgerPropertyTest`, 150 tries — 랜덤 시퀀스에서 원장 SUM==수용분·≤ paid), 동시성 IT(`RefundConcurrencyIT` — 같은 토큰 16스레드 1건·서로 다른 토큰 상한 준수 3건, 5회 반복 무결함), 인수 IT(`RefundAcceptanceIT` — 200/400/404/409). 기존 `PaymentConstraintsIT`는 환불 제약 이전에 맞춰 정리, `PaymentMoneyInvariantPropertyTest`는 원장 property로 대체. 전부 실 Postgres 그린 + ArchUnit·spotless.
