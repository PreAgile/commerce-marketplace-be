...
- **의도적 Scope Out**: 2단계 마감(OPEN→CLOSING→CLOSED)·`CARRIED_OVER` 이월 상태(현재는 CLOSED 단일 + cutoff 스윕으로 late-arrival 흡수라 이월 상태머신 불필요), 지급대행(payout 게이트), 스케줄 자동 마감(현재 운영자 트리거). 환불 음수 라인(M5)·4-way 대사(M6)는 다음 마일스톤.
